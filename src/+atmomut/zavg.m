...
% 2025-11-05    Fixed protection on iz2
%===============================================================================
function [avg, wgt] = zavg(pemod, f, peobs)

if numel(pemod) ~= numel(f) + 1
    error('Size mismatch: PE must have one more element than VV.');
end
...
for j = 1:nlays
    ...
    iz1 = min(floor(dz1), nsig);
    iz2 = min(floor(dz2), nsig);
