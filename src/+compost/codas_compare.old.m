...
%        pegrd = zeros(size(peavg(iob0:iobF,:)));
%        for iob = 1:nobs
%            pegrd(iob,:) = interp1([0, pemod(iob,:), 1e8], ...
%                [1, [1:NLEV+1], NLEV+1], ...
%                peavg(iob0+iob-1,:), 'linear');
%        end
%
%        for kl = 1:size(plavg,2)
%            petop = min(pegrd(:,kl), pegrd(:,kl+1));
%            pebot = max(pegrd(:,kl), pegrd(:,kl+1));
%
%            itop = min(floor(petop), NLEV);
...
%                avg = avg + wgtz.*dpmod(:,ind).*geslmod(:,ind);
...
%        end
%
%        % Fill underground layers with last valid layer
%        for iob = 1:nobs
%            for kl = size(plavg,2):-1:2
%                if ~isnan(geslavg(iob,kl)), break; end
%            end
%            geslavg(iob,kl+1:end) = geslavg(iob,kl);
%        end
    ...
    if ~isfile(fout)
        nccreate(fout, 'mod', 'dimensions',{'nsound',numel(gesobs)}, ...
